    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        <div class="bg-white border-2 rounded-xl border-gray px-5 py-5 mt-2">
            <div class="flex flex-wrap">
                ...
                <div class="w-full max-h-80 overflow-scroll">  
                    <div class="" v-for="item in logs">
                        <p class="bg-gray-400 bg-opacity-20 mt-3 py-1 px-2 rounded me-2">
                            <i>{{ formatDate(item.created_at) }}</i> - {{ parseInfo(item.log)['action'] }} | <a class="text-blue-700" :href="'/user/' + parseInfo(item.log)['user']">Ver usuário</a>                            
                        </p>
                    </div>                    
                </div>   
            </div>
